...
def main():
    ...
    E = EnergyModel(n_experts=256).to(device)
        ...
        for epoch in range(epochs):
            ...
            for i, (imgs, _) in loop:
                ...
                with torch.amp.autocast(device_type=device.type, dtype=torch.float16):
                    ...
                    #     # shape: (bs, C, H, W)
                    #     var = act.var(dim=[0, 2, 3], unbiased=False) + 1e-6
                    #     entropy_term += 0.5 * torch.log(var).mean()

                    # # pairwise distance regularisation
                    flat = gen_imgs.view(bs, -1)
                    dist = torch.pdist(flat, p=2).mean()

                    # std dev regularisation
                    std = gen_imgs.std(dim=[0,2,3]).mean()

                    # gen loss
                    g_loss = energy_fake.mean() - 1.0 * std - 0.05 * dist
                    ...
                    #         # f"Pairwise Distance: {dist.item():.3f}"
                    #         f"Entropy Term: {entropy_term.item():.3f}"
                    #     )
